...
$referral = trim($data['referral'] ?? ''); // <-- new referral field
...
// Insert user including referral
$stmt = $conn->prepare("
    INSERT INTO users 
    (first_name, last_name, name, email, password, referral, created_at, updated_at) 
    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
");
$stmt->bind_param("ssssss", $first_name, $last_name, $fullName, $email, $hashed_password, $referral);
